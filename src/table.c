...
RID table_insert(Table *table, ItemPtr src, short size) {
    ...
    if (addr == -1) {//hash_map is empty
    //insert data
        Block *block = (Block*)get_page(&table->data_pool, table->data_pool.file.length);
        init_block(block);
        idx = new_item(block, src, size);
    //insert hash_map
        short final_size = block->tail_ptr - block->head_ptr;
        off_t final_addr = table->data_pool.file.length - PAGE_SIZE;

        hash_table_insert(&table->fsm_pool, final_size, final_addr);
        get_rid_block_addr(rid) = final_addr;
        release(&table->data_pool, final_addr);
...
void table_delete(Table *table, RID rid) {
    ...
    short idx = get_rid_idx(rid);
    Block *block = (Block*)get_page(&table->data_pool, addr);
    short target_size = block->tail_ptr - block->head_ptr;

    hash_table_pop(&table->fsm_pool, target_size, addr);
    delete_item(block, idx);
    
    target_size = block->tail_ptr - block->head_ptr;
